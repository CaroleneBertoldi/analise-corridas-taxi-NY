...
buscar.rotas <- function(x.origem, y.origem, x.destino, y.destino) {
  q.origem = buscar.quadrante(x.origem, y.origem)
  rota = c()

  for (x in x.origem:x.destino){
    rota = c(rota, buscar.quadrante(x, y.origem))
  }
  
  for (y in (y.origem -1):y.destino){
    rota = c(rota, buscar.quadrante(x.destino, y))
  }

  rota
}

#rota.teste = buscar.rotas(34, 29, 33,38)
rota.teste = buscar.rotas(4, 4, 1, 1)
rota.teste
```

```{r }
corridas.rotas = mutate(corridas.dentro,
                        rota = list(buscar.rotas(pickup_x, pickup_y, dropoff_x, dropoff_y)),
                        locais.origem = list(buscar.locais(pickup_quater)),
                        locais.destino = list(buscar.locais(dropoff_quater)))
...
c = corridas.dentro %>%
  buscar.rotas(pickup_x, pickup_y, dropoff_x, dropoff_y)
c
